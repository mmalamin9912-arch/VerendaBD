  ...
  try {
    ...
    if (supabaseAdmin) {
      try {
        ...
        if (storeSlug) {
          query = query.or(`store_slug.eq.${storeSlug},storeSlug.eq.${storeSlug},merchantId.eq.${storeSlug},merchant_id.eq.${storeSlug}`);
        }
        const { data, error } = await query;
        if (!error && Array.isArray(data)) {
    ...
    const memCats = storeSlug
      ? (inMemoryStore.categories.get(storeSlug) || [])
      : Array.from(inMemoryStore.categories.values()).flat();
...
const handlePostCategories = async (req: express.Request, res: express.Response) => {
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  res.setHeader('Allow', ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS']);
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  ...
  try {
    ...
    const storeSlug = Array.isArray(payload)
      ? (payload[0]?.store_slug || payload[0]?.storeSlug || payload[0]?.merchantId || 'default')
      : (payload.store_slug || payload.storeSlug || payload.merchantId || 'default');

    if (Array.isArray(payload)) {
      inMemoryStore.categories.set(storeSlug, payload);
    } else if (payload && typeof payload === 'object') {
      const existing = inMemoryStore.categories.get(storeSlug) || [];
      ...
      inMemoryStore.categories.set(storeSlug, existing);
    }
    ...
    if (supabaseAdmin && payload) {
      try {
        ...
        console.warn('Supabase upsert categories notice:', e);
      }
    }
    return res.status(200).json(payload);
  } catch (error) {
    console.error('[API Error in handlePostCategories]:', error);
...
app.post('/api/categories', handlePostCategories);
app.put('/api/categories', handlePostCategories);
app.patch('/api/categories', handlePostCategories);
app.delete('/api/categories', handlePostCategories);
app.options('/api/categories', (req, res) => res.status(200).end());
app.options('/api/categories/:merchantId', (req, res) => res.status(200).end());
...
const handleMerchantUpdate = async (req: express.Request, res: express.Response) => {
  ...
  try {
      ...
      if (supabaseAdmin) {
        try {
          const { data, error } = await supabaseAdmin
            .from('merchants')
            .select('*')
            .or(`store_slug.eq.${slug},storeSlug.eq.${slug},slug.eq.${slug},id.eq.${slug}`)
            .maybeSingle();
          if (!error && data) {
            return res.status(200).json({ success: true, data });
          }
        } catch (e) {
          console.warn('[API GET /api/merchants/update] Supabase error:', e);
        }
      }

      const mem = inMemoryStore.merchants.get(slug);
      return res.status(200).json({ success: true, data: mem || null });
    }
    ...
    console.error('[API Error in handleMerchantUpdate]:', err);
    return res.status(200).json({ success: false, error: String(err) });
  }
};

app.all('/api/merchants/update', handleMerchantUpdate);
app.get('/api/merchants/update', handleMerchantUpdate);
...
app.delete('/api/merchants/update', handleMerchantUpdate);
app.options('/api/merchants/update', (req, res) => res.status(200).end());

app.all('/api/merchants', handleMerchantUpdate);
app.get('/api/merchants', handleMerchantUpdate);
